...
if __name__ == '__main__':
    with app.app_context():
        ...
        # Set owner privileges for configured owner IDs
        from config import Config
        from models import User
        for owner_id in Config.OWNER_IDS:
            owner_user = db.session.query(User).filter_by(telegram_id=str(owner_id)).first()
            if owner_user and owner_user.subscription_type != SubscriptionType.OWNER:
                owner_user.subscription_type = SubscriptionType.OWNER
                db.session.commit()
                logger.info(f"Owner privileges set for user {owner_id}")
